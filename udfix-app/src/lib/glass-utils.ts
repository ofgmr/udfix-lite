...
import { resolveUyapSigningErrorPresentation } from '../utils/udfSignatureState'
...
/** Sonner toast API (glass-themed via `components/ui/sonner.tsx` + `index.css`). */
export { sonnerToast as toast }

const SIGNING_TOAST_DURATION_MS = 8000

/** User-visible e-imza failure (summary title + API/operator detail when available). */
export function toastUyapSigningError(
  error: unknown,
  options?: { title?: string } & ExternalToast,
): void {
  const { summary, detail } = resolveUyapSigningErrorPresentation(error)
  const { title = summary, ...rest } = options ?? {}
  sonnerToast.error(title, {
    description: detail ?? undefined,
    duration: SIGNING_TOAST_DURATION_MS,
    ...rest,
  })
}

/** Signed UDF export/save failure with mapped signing error text. */
export function toastUdfExportError(error: unknown, options?: ExternalToast): void {
  const { summary, detail } = resolveUyapSigningErrorPresentation(error)
  sonnerToast.error('UDF dışa aktarımı başarısız', {
    description: composeUdfExportErrorDescription(summary, detail),
    duration: SIGNING_TOAST_DURATION_MS,
    ...options,
  })
}

function composeUdfExportErrorDescription(summary: string, detail: string | null): string {
  if (!detail) {
    return summary
  }
  if (detail.toLowerCase() === summary.toLowerCase()) {
    return summary
  }
