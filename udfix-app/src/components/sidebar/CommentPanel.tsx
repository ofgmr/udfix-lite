...
const CommentPanel: React.FC<CommentPanelProps> = ({ editor: editorProp }) => {
    ...
    const startDraft = useDocumentCommentsStore((s) => s.startDraft);
    const setDocumentId = useDocumentCommentsStore((s) => s.setDocumentId);
    ...
    useEffect(() => {
        if (editor) {
            highlightComment(editor, highlightedCommentId || activeCommentId);
        }
    }, [highlightedCommentId, activeCommentId, editor]);
